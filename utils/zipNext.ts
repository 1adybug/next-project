...
export async function zipNext(id: string, current?: string) {
    ...
    const fileName = dir2.find(file => file.toLowerCase().endsWith(".zip") || file.toLowerCase().endsWith(".7z"))
    if (!fileName) throw new Error("未找到压缩文件")
    await unzip({
        input: join(DIR, id, "releases", current, fileName),
        output: join(DIR, id, "main")
    })
}
